/**
 * Response shapes for the venue-owner API routes.
 *
 * Same reasoning as api/waiter.ts: this is the contract between the server and
 * both clients, so it lives here rather than in either app. Types only.
 *
 * The analytics types stay in apps/web — they are re-exported from a web lib
 * module and the mobile app does not render charts (recharts has no React Native
 * build, and admin/analytics is web-only by design).
 */

import type { VenueZoneInsights } from "./zones";
