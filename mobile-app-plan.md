...
Status: **Phase 0a + 0b done** (monorepo, `@ekonobar/shared` populated), **Phase 1a done** (bearer auth), **Phase 2 done** (native push), **Phase 4 done** (Expo SDK 57 app boots and signs in against the real backend), **Phase 5 done** (waiter screens: Pregled, Poslovi, Smene, Recenzije, Passport). Next: **Phase 6** (venue owner screens). **Phase 1b** (native OAuth) and **Phase 3** (bootstrap endpoint, cursor pagination) still outstanding.
