...
**Reach coverage:** `GET /api/waiters/coverage` (`VENUE_OWNER`/`HEADHUNTER`, Redis-cached 300s) returns `[{ municipality, availableCount }]` for every opština — how many available waiters declared reach there. Aggregate only (no names/coordinates); a waiter with two municipalities counts for both (it measures "how many will come here", not a partition). Computed with one `findMany` of available waiters' reach arrays tallied in-process — Prisma can't `GROUP BY` an array element. Rendered as the ranked `ReachCoveragePanel` bar ladder in the venue Discover section; clicking a bar drives the same `municipality` filter the dropdown does. A choropleth map layer is a future add — it needs correct Belgrade opština boundary GeoJSON (RentCheck's covers only 15 of the 17 and includes a non-Belgrade one, so it isn't a drop-in).
